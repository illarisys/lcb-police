...
import lcb.com.pe.service.PermisoRepositorioService;
...
public class OAuth2SecurityExpressionMethods {
	 ...
	 */
	public boolean hasOption(String tabla) {
		PermisoRepositorioService bean = new PermisoRepositorioService();
		AutowireCapableBeanFactory factory = ApplicationContextProvider.getApplicationContext().getAutowireCapableBeanFactory();
		factory.autowireBean( bean );
		factory.initializeBean(bean,"permisoRepositorioService");
		User user = (User) authentication.getPrincipal();
		return bean.findPermisoByRolUsuario(user.getUsername(), tabla);
	}
